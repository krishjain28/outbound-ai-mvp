...
async function authenticateUser() {
  ...
  try {
    ...
    const signinResponse = await axios.post('http://localhost:5001/api/auth/signin', {
    ...
    if (signinError.response?.status === 401) {
      ...
      try {
        ...
        const signupResponse = await axios.post('http://localhost:5001/api/auth/signup', TEST_USER);
        ...
        if (signupError.response?.data?.message?.includes('already exists')) {
          colorLog('red', '❌ User exists but signin failed. Check credentials.');
...
async function makeLiveTestCall() {
  ...
  try {
    ...
    try {
      const configResponse = await axios.get('http://localhost:5001/api/calls/config-status', {
      ...
      console.log('Config error:', configError.response?.data || configError.message);
    }
    ...
    const callData = {
      ...
      leadName: leadName,
      aiPrompt: 'You are Mike, a friendly sales rep testing the optimized call system. Keep responses short and natural. Ask how they are doing and mention this is a test call to verify the system optimizations are working properly.'
    };
    
    const callResponse = await axios.post('http://localhost:5001/api/calls/initiate', callData, {
      headers: {
    ...
    colorLog('cyan', '- "No, not interested" (should ask about current methods)\n');
    ...
    const monitorInterval = setInterval(async () => {
      try {
        ...
        // Check call status
        const statusResponse = await axios.get(`http://localhost:5001/api/calls/${callId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        if (call.conversation && call.conversation.length > 0) {
          colorLog('green', `💬 Conversation turns: ${call.conversation.length}`);
        ...
        if (call.status === 'completed' || call.status === 'failed') {
          ...
          colorLog('green', `Conversation turns: ${call.conversation?.length || 0}`);
    ...
    if (error.response?.status === 401) {
      colorLog('yellow', '🔐 Authentication failed. Check test user credentials.');
    } else if (error.response?.status === 500) {
      colorLog('yellow', '🔧 Server error. Check if all environment variables are configured.');
      colorLog('yellow', '📝 Required: TELNYX_API_KEY, TELNYX_PHONE_NUMBER, OPENAI_API_KEY');
      colorLog('yellow', '📝 Optional: DEEPGRAM_API_KEY, ELEVENLABS_API_KEY');
    } else if (error.response?.status === 503) {
      colorLog('yellow', '🗄️ Database connection issue. Check MongoDB connection.');
    }
