...
async function testOptimizedCallSystem() {
  ...
  try {
    ...
    if (missingRequired.length > 0) {
      colorLog('red', `❌ Missing required environment variables: ${missingRequired.join(', ')}`);
      colorLog('yellow', '   Please configure these in your .env file');
      return;
    }
    
    if (missingOptional.length > 0) {
      colorLog('yellow', `⚠️  Missing optional environment variables: ${missingOptional.join(', ')}`);
      colorLog('yellow', '   These enable enhanced features but system will work without them');
    }
    
    // 3. Test Telnyx API
    colorLog('blue', '\n📞 Testing Telnyx API...');
    try {
      const telnyxResponse = await axios.get('https://api.telnyx.com/v2/available_phone_numbers', {
        headers: {
      ...
      colorLog('green', `📱 Phone number configured: ${process.env.TELNYX_PHONE_NUMBER || 'Not set'}`);
      results.telnyx = true;
    } catch (error) {
    ...
    if (process.env.DEEPGRAM_API_KEY) {
      ...
      try {
        ...
        const response = await deepgram.manage.getProjectBalances(process.env.DEEPGRAM_PROJECT_ID || 'default');
    ...
    try {
      const ngrokResponse = await axios.get(process.env.BACKEND_URL + '/health', {
        timeout: 10000
      });
      
      if (ngrokResponse.status === 200) {
        colorLog('green', '✅ Ngrok tunnel is working');
        colorLog('green', `🔗 Webhook URL: ${process.env.BACKEND_URL}/api/calls/webhook`);
        results.ngrok = true;
      }
    } catch (error) {
      colorLog('red', '❌ Ngrok tunnel test failed');
      colorLog('red', `   Error: ${error.message}`);
      colorLog('yellow', '   Make sure ngrok is running and BACKEND_URL is correct');
    }
    
    // 8. Test MongoDB Connection
    colorLog('blue', '\n🗄️ Testing MongoDB Connection...');
    try {
      const mongoResponse = await axios.get('http://localhost:5001/api/auth/health', {
        timeout: 5000
      });
    ...
    try {
      const speechServiceResponse = await axios.get('http://localhost:5001/api/calls/config-status', {
        timeout: 5000
      });
      ...
      if (speechServiceResponse.status === 200) {
        ...
        if (config.enhanced_speech_recognition) {
          colorLog('green', '🎙️ Enhanced speech recognition (Deepgram) enabled');
        }
    ...
    if (passedTests >= 4) { // Backend, Telnyx, OpenAI, and either Ngrok or MongoDB
      ...
      colorLog('yellow', '3. Verify reduced voice delay and improved responses');
