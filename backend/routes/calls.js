...
router.post(
  ...
  [
    ...
    body('phoneNumber')
      ...
      .withMessage(
        'Please provide a valid international phone number (7-15 digits)'
      ),
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    console.log(
      `🔄 Processing webhook: ${data.event_type} for call ${callControlId}`
    );
    ...
    if (!call && clientState) {
      ...
      if (call) {
        console.log(
          `📝 Updating call ${call._id} with callId: ${callControlId}`
        );
    ...
    if (!call) {
      console.error('❌ Call not found for webhook:', {
        callControlId,
        clientState,
    ...
    console.log(
      `✅ Found call ${call._id} (status: ${call.status}) for webhook ${data.event_type}`
    );
    ...
    switch (data.event_type) {
      ...
      case 'call.answered':
        console.log(
          `Call ${callControlId} answered, starting optimized AI conversation`
        );
        ...
        setTimeout(async () => {
          try {
            ...
            const speechResult = await speechService.speakText(
              callControlId,
              welcomeMessage
            );
      ...
      case 'call.hangup': {
        ...
        console.log(
          `✅ Call ${callControlId} completed, duration: ${duration}s`
        );
        break;
      }
      ...
      case 'call.speak.ended':
        console.log(
          `🎤 AI finished speaking for call ${callControlId}, starting optimized speech recognition`
        );
        ...
        setTimeout(async () => {
          try {
            ...
            if (
              ...
              console.log(
                `Call ${callControlId} is not active, skipping speech recognition`
              );
              return;
            }
            ...
            console.log(
              `🎙️ Starting optimized speech recognition for call ${callControlId}`
            );

            const speechResult =
              await speechService.startRealTimeSpeechRecognition(
                callControlId,
                call,
                async transcript => {
                  ...
                  const aiResponse = await generateQuickResponse(
                    call,
                    transcript
                  );
    ...
    switch (data.event_type) {
      case 'stream.transcription':
        if (data.payload?.transcription?.text) {
          ...
          console.log(
            `📝 ${isFinal ? 'FINAL' : 'interim'} transcription for call ${callControlId}: "${transcriptionText}"`
          );

          // Only process final transcriptions with meaningful content
          if (isFinal && transcriptionText.length > 2) {
            console.log(
              `✅ Processing customer response: "${transcriptionText}"`
            );
            ...
            try {
              ...
              console.log(
                `🛑 Stopped speech recognition for call ${callControlId}`
              );
            } catch (stopError) {
              console.error(
                'Error stopping speech stream:',
                stopError.response?.data || stopError.message
              );
            }
      ...
      case 'stream.started':
        console.log(
          `🎤 Speech recognition stream started for call: ${callControlId}`
        );
        break;

      case 'stream.stopped':
        console.log(
          `⏹️ Speech recognition stream stopped for call: ${callControlId}`
        );
        break;

      case 'stream.error': {
        console.error(
          `❌ Speech recognition error for call ${callControlId}:`,
          data.payload
        );
...
async function generateQuickResponse(call, userResponse) {
  try {
    ...
    if (
      ...
      lowerResponse.includes('not a good time')
    ) {
...
async function speakWithNaturalVoice(callControlId, message) {
  try {
    ...
    if (call.status === 'completed' || call.status === 'failed') {
      console.log(
        `Call ${callControlId} is already ${call.status}, skipping TTS`
      );
      return;
    }
    ...
    console.error(
      `TTS Error for call ${callControlId}:`,
...
function addNaturalPauses(message) {
  ...
  naturalMessage = naturalMessage.replace(
    /Hi ([^,!]+)[,!]/g,
    'Hi $1!<break time="0.4s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Hey ([^,!]+)[,!]/g,
    'Hey $1!<break time="0.3s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Hello ([^,!]+)[,!]/g,
    'Hello $1!<break time="0.4s"/>'
  );

  // Natural conversation fillers with pauses (human speech patterns)
  naturalMessage = naturalMessage.replace(
    /You know,/g,
    'You know,<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /I mean,/g,
    'I mean,<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Actually,/g,
    'Actually,<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Honestly,/g,
    'Honestly,<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(/So,/g, 'So,<break time="0.2s"/>');
  naturalMessage = naturalMessage.replace(
    /Well,/g,
    'Well,<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(/Oh,/g, 'Oh,<break time="0.2s"/>');
  naturalMessage = naturalMessage.replace(
    /Like,/g,
    'Like,<break time="0.1s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Right,/g,
    'Right,<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Okay,/g,
    'Okay,<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Yeah,/g,
    'Yeah,<break time="0.1s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Totally,/g,
    'Totally,<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Absolutely,/g,
    'Absolutely,<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /For sure,/g,
    'For sure,<break time="0.2s"/>'
  );
  ...
  naturalMessage = naturalMessage.replace(
    /Let me think/g,
    'Let me think<break time="0.3s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Let me see/g,
    'Let me see<break time="0.2s"/>'
  );

  // Authentic reactions with emphasis and natural timing
  naturalMessage = naturalMessage.replace(
    /Oh really\?/g,
    '<emphasis level="moderate">Oh really?</emphasis><break time="0.4s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /That's interesting!/g,
    '<emphasis level="moderate">That\'s interesting!</emphasis><break time="0.3s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Wow,/g,
    '<emphasis level="moderate">Wow,</emphasis><break time="0.3s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Cool!/g,
    '<emphasis level="moderate">Cool!</emphasis><break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Nice!/g,
    '<emphasis level="moderate">Nice!</emphasis><break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /No way!/g,
    '<emphasis level="moderate">No way!</emphasis><break time="0.3s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Really\?/g,
    '<emphasis level="moderate">Really?</emphasis><break time="0.3s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Seriously\?/g,
    '<emphasis level="moderate">Seriously?</emphasis><break time="0.3s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /That's crazy!/g,
    '<emphasis level="moderate">That\'s crazy!</emphasis><break time="0.3s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /I get it/g,
    '<emphasis level="moderate">I get it</emphasis><break time="0.2s"/>'
  );

  // Natural acknowledgments
  naturalMessage = naturalMessage.replace(
    /Mm-hmm/g,
    'Mm-hmm<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Uh-huh/g,
    'Uh-huh<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Got it/g,
    'Got it<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /Makes sense/g,
    'Makes sense<break time="0.2s"/>'
  );
  naturalMessage = naturalMessage.replace(
    /I hear you/g,
    'I hear you<break time="0.2s"/>'
  );
  ...
  naturalMessage = naturalMessage.replace(
    /Here's the thing/g,
    '<break time="0.3s"/>Here\'s the thing'
  );
  naturalMessage = naturalMessage.replace(
    /You know what/g,
    '<break time="0.2s"/>You know what'
  );
  naturalMessage = naturalMessage.replace(
    /I'm curious/g,
    '<break time="0.2s"/>I\'m curious'
  );
  naturalMessage = naturalMessage.replace(
    /Let me ask you/g,
    '<break time="0.3s"/>Let me ask you'
  );
  naturalMessage = naturalMessage.replace(
    /Can I ask/g,
    '<break time="0.2s"/>Can I ask'
  );
  naturalMessage = naturalMessage.replace(
    /Tell me/g,
    '<break time="0.2s"/>Tell me'
  );
  naturalMessage = naturalMessage.replace(
    /So tell me/g,
    '<break time="0.2s"/>So tell me'
  );
  naturalMessage = naturalMessage.replace(
    /Actually, that reminds me/g,
    '<break time="0.2s"/>Actually, that reminds me'
  );
  ...
  naturalMessage = naturalMessage.replace(
    /haha/g,
    '<prosody rate="fast" pitch="+10%">haha</prosody>'
  );
  naturalMessage = naturalMessage.replace(
    /oh my gosh/g,
    '<prosody pitch="+15%">oh my gosh</prosody>'
  );
  naturalMessage = naturalMessage.replace(
    /that's awesome/g,
    '<prosody pitch="+10%">that\'s awesome</prosody>'
  );
  naturalMessage = naturalMessage.replace(
    /that's great/g,
    '<prosody pitch="+10%">that\'s great</prosody>'
  );

  // Slow down for emphasis on important words but keep it natural
  naturalMessage = naturalMessage.replace(
    /(customers|business|website|results|help|solution)/g,
    '<prosody rate="0.9">$1</prosody>'
  );
...
async function startListeningForResponse(
  callControlId,
  call,
  conversationTurn = 1
) {
  try {
    ...
    console.log(
      `Starting recording for call ${callControlId}, turn ${conversationTurn}`
    );
...
async function startRealSpeechRecognition(callControlId, call) {
  try {
    ...
    if (
      ...
      console.log(
        `Call ${callControlId} is not active, skipping real speech recognition`
      );
      return;
    }

    console.log(
      `🎙️ Starting real-time speech recognition for call ${callControlId}`
    );
    ...
    console.log(
      `✅ Optimized speech recognition started for call ${callControlId}`
    );
    ...
    setTimeout(async () => {
      try {
        ...
        if (stillActive && stillActive.status === 'in_progress') {
          console.log(
            `⏰ Speech recognition timeout for call ${callControlId}, prompting customer`
          );
          await handleSilentCustomer(
            callControlId,
            call,
            Math.floor(call.conversation.length / 2) + 1
          );
        }
      } catch (error) {
    ...
    console.error(
      'Real speech recognition start error:',
      error.response?.data || error.message
    );
    ...
    try {
      console.log(
        `🔄 Fallback: Starting basic recording for call ${callControlId}`
      );
      await telnyxAPI.post(`/calls/${callControlId}/actions/start_recording`, {
        format: 'mp3',
        channels: 'dual',
...
async function handleUserResponse(
  ...
  try {
    console.log(
      `Processing user response for call ${callControlId}, turn ${conversationTurn}: "${userResponse}"`
    );
    ...
    if (
      ...
      console.log(
        `Call ${callControlId} is not active, skipping response processing`
      );
      return;
    }
    ...
    const aiResponse = await generateNaturalResponse(
      call,
      userResponse,
      'conversation'
    );
    ...
    try {
      ...
      console.error(
        'TTS Error in conversation:',
        speakError.response?.data || speakError.message
      );
    ...
    if (conversationLength >= 8) {
      // If conversation is progressing well
      await call.updateOne({
        outcome: 'engaged',
    ...
    if (conversationTurn >= 5) {
      console.log(
        `Ending conversation for call ${callControlId} after ${conversationTurn} turns`
      );
      setTimeout(async () => {
        try {
          ...
          console.error(
            'Hangup error:',
            hangupError.response?.data || hangupError.message
          );
        }
