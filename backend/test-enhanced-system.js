...
async function testEnhancedSystem() {
  ...
  try {
    ...
    try {
      ...
      colorLog('red', '❌ Backend not accessible. Please start the backend server.');
    ...
    if (requiredMissing.length === 0) {
      ...
      colorLog('red', `❌ Missing required environment variables: ${requiredMissing.join(', ')}`);
    }
    ...
    if (enhancedMissing.length === 0) {
      ...
      colorLog('yellow', `⚠️  Optional enhanced APIs not configured: ${enhancedMissing.join(', ')}`);
    }
    ...
    if (process.env.TELNYX_API_KEY) {
      ...
      try {
        ...
        if (phoneNumbers.length > 0) {
          const configuredNumber = phoneNumbers.find(num => num.phone_number === process.env.TELNYX_PHONE_NUMBER);
          if (configuredNumber) {
            colorLog('green', `✅ Configured phone number ${process.env.TELNYX_PHONE_NUMBER} is available`);
          } else {
            colorLog('yellow', `⚠️  Configured phone number ${process.env.TELNYX_PHONE_NUMBER} not found in account`);
          }
        }
        ...
        colorLog('red', `   Error: ${error.response?.data?.errors?.[0]?.detail || error.message}`);
      }
    }
    ...
    if (process.env.OPENAI_API_KEY) {
      ...
      try {
        ...
        if (gpt4Models.length > 0) {
          ...
          colorLog('yellow', `⚠️  GPT-4 access not available, will use GPT-3.5`);
        }
        
        results.openai = true;
      } catch (error) {
        colorLog('red', '❌ OpenAI API connection failed');
        colorLog('red', `   Error: ${error.response?.data?.error?.message || error.message}`);
      }
    }
    ...
    if (process.env.DEEPGRAM_API_KEY) {
      ...
      try {
        ...
        colorLog('red', `   Error: ${error.response?.data?.message || error.message}`);
      }
    }
    ...
    if (process.env.ELEVENLABS_API_KEY) {
      ...
      try {
        ...
        if (process.env.ELEVENLABS_VOICE_ID) {
          const configuredVoice = voices.voices?.find(voice => voice.voice_id === process.env.ELEVENLABS_VOICE_ID);
          if (configuredVoice) {
            colorLog('green', `✅ Configured voice "${configuredVoice.name}" is available`);
          } else {
            colorLog('yellow', `⚠️  Configured voice ID not found, will use default`);
          }
        }
    ...
    if (results.backend) {
      ...
      try {
        ...
        const testContext = conversationService.initializeConversation(testCallId, 'Test User');
        ...
        if (testContext && testContext.callId === testCallId) {
          ...
          const openingMessage = conversationService.generateOpeningMessage('Test User');
    ...
    if (results.backend) {
      ...
      try {
        ...
        const response = await axios.get('http://localhost:5001/api/calls/config-status', {
          headers: {
        ...
        if (response.status === 200 || response.status === 401) {
          ...
          if (response.status === 401) {
            colorLog('yellow', '⚠️  Authentication required (expected for security)');
          }
        }
      } catch (error) {
        if (error.response?.status === 401) {
          colorLog('green', '✅ Config status endpoint accessible (auth required)');
        } else {
    ...
    colorLog('blue', `Backend Health: ${results.backend ? '✅ PASS' : '❌ FAIL'}`);
    colorLog('blue', `Required APIs: ${results.requiredApis ? '✅ PASS' : '❌ FAIL'}`);
    colorLog('blue', `Enhanced APIs: ${results.enhancedApis ? '✅ PASS' : '⚠️  PARTIAL'}`);
    colorLog('blue', `Telnyx API: ${results.telnyx ? '✅ PASS' : '❌ FAIL'}`);
    colorLog('blue', `OpenAI API: ${results.openai ? '✅ PASS' : '❌ FAIL'}`);
    colorLog('blue', `Deepgram API: ${results.deepgram ? '✅ PASS' : '⚠️  NOT CONFIGURED'}`);
    colorLog('blue', `ElevenLabs API: ${results.elevenlabs ? '✅ PASS' : '⚠️  NOT CONFIGURED'}`);
    colorLog('blue', `Enhanced Services: ${results.services ? '✅ PASS' : '❌ FAIL'}`);
    
    colorLog('bright', `\n🎯 Overall Success Rate: ${successRate}%`);
    
    if (results.backend && results.requiredApis && results.telnyx && results.openai && results.services) {
      ...
      if (results.deepgram && results.elevenlabs) {
        ...
        colorLog('green', '   • Ultra-realistic voice synthesis with ElevenLabs');
        ...
        colorLog('yellow', '   Add Deepgram and ElevenLabs API keys for full features');
      }
    } else {
      colorLog('red', '❌ SYSTEM NOT READY - Please fix the failed tests above');
    }
    
    // 10. Next Steps
    colorLog('bright', '\n🔧 NEXT STEPS:');
    if (!results.backend) {
      colorLog('yellow', '1. Start the backend server: cd backend && npm run dev');
    }
    if (!results.requiredApis) {
      colorLog('yellow', '2. Configure required environment variables in backend/.env');
    }
    if (!results.deepgram) {
      colorLog('yellow', '3. Add DEEPGRAM_API_KEY for real-time speech recognition');
    }
