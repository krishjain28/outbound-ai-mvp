...
const CallPage: React.FC = () => {
  ...
  const isCallActive = useMemo(() => 
    currentCall && ['initiated', 'ringing', 'answered', 'in_progress'].includes(currentCall.status),
    [currentCall]
  );
  ...
  const formatPhoneNumber = useCallback((value: string, format: string): string => {
  ...
  const handleCountryChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (phoneNumber) {
      const newCountryData = COUNTRIES.find(c => c.code === newCountry) || COUNTRIES[0];
      const cleaned = phoneNumber.replace(/\D/g, '');
  ...
  const checkConfigStatus = useCallback(async () => {
    try {
      ...
      if (!response.data.configured) {
        setConfigError('API configuration incomplete. Please set up your Telnyx and OpenAI API keys.');
      }
    } catch (error: any) {
      console.error('Error checking config status:', error);
      setConfigError('Unable to check API configuration. Please ensure the backend is running.');
    }
  }, []);
  ...
  const fetchRecentCalls = useCallback(async () => {
    try {
      ...
      const successful = calls.filter((call: Call) => call.status === 'completed').length;
      const totalDuration = calls.reduce((sum: number, call: Call) => sum + (call.duration || 0), 0);
      ...
      setCallStats({
        ...
        conversionRate: calls.length > 0 ? Math.round((meetings / calls.length) * 100) : 0
      });
    } catch (error) {
  ...
  const StatusBadge: React.FC<{ status: Call['status'] }> = ({ status }) => {
    const statusConfig = {
      initiated: { color: 'bg-blue-100 text-blue-800', icon: ArrowPathIcon, text: 'Initiating' },
      ringing: { color: 'bg-yellow-100 text-yellow-800', icon: PhoneIcon, text: 'Ringing' },
      answered: { color: 'bg-green-100 text-green-800', icon: PhoneIconSolid, text: 'Answered' },
      in_progress: { color: 'bg-green-100 text-green-800', icon: PhoneIconSolid, text: 'In Progress' },
      ...
      busy: { color: 'bg-orange-100 text-orange-800', icon: PhoneXMarkIcon, text: 'Busy' },
      no_answer: { color: 'bg-gray-100 text-gray-800', icon: PhoneXMarkIcon, text: 'No Answer' }
    };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">AI Sales Calls</h1>
                <p className="text-sm text-gray-500">Automated lead qualification and meeting booking</p>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-600">{callStats.averageDuration}s</div>
                  <div className="text-sm text-gray-500">Avg Duration</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-orange-600">{callStats.conversionRate}%</div>
                  ...
                  <div className="flex-1">
                    ...
                    {!configStatus.configured && (
                      <div className="mt-2 text-xs text-red-600">
                        <p><strong>Missing Required APIs:</strong></p>
                        <ul className="list-disc list-inside ml-2">
                          {!configStatus.config?.telnyx_api_key && <li>Telnyx API Key</li>}
                          {!configStatus.config?.telnyx_phone_number && <li>Telnyx Phone Number</li>}
                          {!configStatus.config?.openai_api_key && <li>OpenAI API Key</li>}
                        </ul>
                      </div>
                    )}
                    ...
                    {configStatus.configured && !configStatus.enhanced && (
                      <div className="mt-2 text-xs text-blue-600">
                        <p><strong>Optional Enhancement APIs:</strong></p>
                        <ul className="list-disc list-inside ml-2">
                          {!configStatus.config?.deepgram_api_key && <li>Deepgram API Key (for real-time speech recognition)</li>}
                          {!configStatus.config?.elevenlabs_api_key && <li>ElevenLabs API Key (for ultra-realistic voice)</li>}
                        </ul>
                      </div>
                    )}
                  </div>
            ...
            {configError && !configStatus && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                <div className="flex">
                  ...
                  <div>
                    <h3 className="text-sm font-medium text-red-800">Configuration Check Failed</h3>
            ...
            {/* Call Interface */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-6">Make a Call</h2>
                    ...
                    <div className="flex items-center">
                      ...
                      <div>
                        <p className="font-medium text-blue-900">
                          Calling {currentCall.leadName} ({currentCall.phoneNumber})
                        </p>
              ...
              <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex">
                      ...
                      <input
                        ...
                        placeholder={selectedCountryData.format.replace(/#/g, '0')}
                        disabled={!!isCallActive}
                ...
                <div>
                  ...
                  <div className="mt-2 text-xs text-gray-500">
                    <p className="font-medium">🎯 Professional SDR with Enhanced Features:</p>
                    <ul className="list-disc list-inside mt-1 space-y-1">
                      <li>Natural human conversation with GPT-4 intelligence</li>
                      {configStatus?.config?.elevenlabs_api_key && (
            ...
            {/* Live Conversation */}
            {currentCall && currentCall.conversation && currentCall.conversation.length > 0 && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-4 max-h-96 overflow-y-auto">
                  {currentCall.conversation.map((entry, index) => (
                    <div key={index} className={`flex ${entry.role === 'ai' ? 'justify-start' : 'justify-end'}`}>
                      <div className={`max-w-md px-4 py-3 rounded-lg shadow-sm ${
                        ...
                        <div className="flex items-center mb-2">
                          <span className="text-xs font-semibold uppercase tracking-wide">
                            {entry.role === 'ai' ? '🗣️ Mike (Human-like AI)' : '👤 Customer'}
                          </span>
                ...
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-yellow-800">
                      This is a live demonstration of ultra-natural human-like AI conversation with authentic speech patterns
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                >
                  <ArrowPathIcon className={`h-4 w-4 ${isLoadingCalls ? 'animate-spin' : ''}`} />
                </button>
              </div>
              ...
              {isLoadingCalls ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-gray-400">Your recent calls will appear here</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {recentCalls.map(call => (
                    <div key={call._id} className="p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
